...
// Add a test for the new Content Pipeline
function test_content_pipeline() {
    echo '<h2>Testing Content Pipeline</h2>';
    
    // Check if URL parameter is set
    if (!isset($_GET['url'])) {
        echo '<div style="background-color: #f8d7da; color: #721c24; padding: 10px; margin: 10px 0; border-radius: 3px;">Please provide a URL parameter to test.</div>';
        echo '<p>Example: ' . htmlspecialchars($_SERVER['PHP_SELF'] . '?test=pipeline&url=https://example.com') . '</p>';
        return;
    }
    
    $url = esc_url_raw($_GET['url']);
    echo '<p>Testing pipeline for URL: ' . htmlspecialchars($url) . '</p>';
    
    try {
        // Load the Content Pipeline class
        ...
        if (is_wp_error($result)) {
            echo '<div style="background-color: #f8d7da; color: #721c24; padding: 10px; margin: 10px 0; border-radius: 3px;">Error: ' . esc_html($result->get_error_message()) . '</div>';
            return;
        }
        
        // Show success message
        echo '<div style="background-color: #d4edda; color: #155724; padding: 10px; margin: 10px 0; border-radius: 3px;">
            Post created successfully! 
            <ul>
                <li>Post ID: ' . esc_html($result['post_id']) . '</li>
                <li><a href="' . esc_url($result['edit_url']) . '" target="_blank">Edit Post</a></li>
                <li><a href="' . esc_url($result['view_url']) . '" target="_blank">View Post</a></li>
            </ul>
        </div>';
        
    } catch (Exception $e) {
        echo '<div style="background-color: #f8d7da; color: #721c24; padding: 10px; margin: 10px 0; border-radius: 3px;">Exception: ' . esc_html($e->getMessage()) . '</div>';
    }
}
