...
class Post_Generator {
    ...
    /**
     * Render a block preview for LLM testing
     * 
     * @param array $block Block data from LLM
     * @return string HTML representation of the block
     */
    public function render_block_preview($block) {
        if (empty($block) || !is_array($block)) {
            return '<div class="utg-preview-block-error">Invalid block data</div>';
        }
        
        $block_type = isset($block['type']) ? sanitize_text_field($block['type']) : 'unknown';
        $block_html = '<div class="utg-preview-block">';
        $block_html .= '<span class="utg-preview-block-type">' . esc_html($block_type) . '</span>';
        
        switch ($block_type) {
            case 'paragraph':
                $content = isset($block['content']) ? $block['content'] : '';
                $block_html .= '<p>' . $this->process_inline_elements($content) . '</p>';
                break;
            ...
            case 'list':
                ...
                $list_type = isset($block['listType']) && $block['listType'] === 'ordered' ? 'ol' : 'ul';
                
                $block_html .= '<' . $list_type . '>';
                foreach ($items as $item) {
                    $block_html .= '<li>' . $this->process_inline_elements($item) . '</li>';
                }
                $block_html .= '</' . $list_type . '>';
                break;
            ...
            case 'image':
                ...
                $caption = isset($block['caption']) ? $this->process_inline_elements($block['caption']) : '';
                
                $block_html .= '<figure>';
                if (!empty($url)) {
                    $block_html .= '<img src="' . $url . '" alt="' . $alt . '" />';
                } else {
                    $block_html .= '<div class="utg-preview-image-placeholder">Image placeholder (URL missing)</div>';
                }
                
                if (!empty($caption)) {
                    $block_html .= '<figcaption>' . $caption . '</figcaption>';
                }
                $block_html .= '</figure>';
                break;
            ...
            case 'quote':
                $content = isset($block['content']) ? $block['content'] : '';
                $citation = isset($block['citation']) ? $block['citation'] : '';
                
                $block_html .= '<blockquote>';
                $block_html .= '<p>' . $this->process_inline_elements($content) . '</p>';
                if (!empty($citation)) {
                    $block_html .= '<cite>' . esc_html($citation) . '</cite>';
                }
                $block_html .= '</blockquote>';
                break;
            ...
            case 'code':
                $content = isset($block['content']) ? $block['content'] : '';
                $language = isset($block['language']) ? $block['language'] : '';
                
                $block_html .= '<pre';
                if (!empty($language)) {
                    $block_html .= ' class="language-' . esc_attr($language) . '"';
                }
                $block_html .= '><code>' . esc_html($content) . '</code></pre>';
                break;
                
            case 'table':
                $rows = isset($block['rows']) ? $block['rows'] : [];
                $block_html .= '<table border="1" cellpadding="5" cellspacing="0">';
                ...
                if (!empty($rows)) {
                    ...
                    foreach ($rows as $row) {
                        ...
                        foreach ($row as $cell) {
                            $tag = $first_row ? 'th' : 'td';
                            $block_html .= '<' . $tag . '>' . $this->process_inline_elements($cell) . '</' . $tag . '>';
                        }
                        $block_html .= '</tr>';
                        $first_row = false;
                    }
            ...
            default:
                // For unknown block types, display content as a paragraph if available
                if (isset($block['content'])) {
                    $block_html .= '<p>' . $this->process_inline_elements($block['content']) . '</p>';
                } else {
                    $block_html .= '<p><em>Unsupported block type: ' . esc_html($block_type) . '</em></p>';
                }
                break;
        }
     ...
     * 
     * @param string $content Text content that might contain inline markup
     * @return string Processed HTML
     */
    private function process_inline_elements($content) {
        if (empty($content)) {
            return '';
        }
        
        // Basic support for bold, italic, links
        $processed = $content;
        
        // Process links formatted as [text](url)
        $processed = preg_replace_callback('/\[([^\]]+)\]\(([^)]+)\)/', function($matches) {
            $text = $matches[1];
            $url = esc_url($matches[2]);
            return '<a href="' . $url . '">' . esc_html($text) . '</a>';
        }, $processed);
