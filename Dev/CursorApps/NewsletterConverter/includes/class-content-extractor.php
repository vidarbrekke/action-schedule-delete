...
class Content_Extractor {
     ...
     */
    public function extract( $url, $cleaning_level = 'medium' ) {
        if (empty($url)) {
        ...
        // Always use medium cleaning level for better HTML quality
        if ($cleaning_level !== 'medium') {
            $cleaning_level = 'medium';
            error_log('UTG: Forcing medium cleaning mode for better quality HTML');
        }
        ...
        try {
            ...
            try {
                ...
                if (!empty($text)) {
                    ...
                    // Always apply medium level cleaning for best results
