# WP Customer AI Chatbot – Environment & Server Setup
...
- **Plugin Directory:** `/home/staging/public_html/wp-content/plugins/wp-customer-ai-chatbot/`
...
- **WooCommerce/Plugin Logs:** `/home/staging/public_html/wp-content/uploads/wc-logs/` (clear with `find ... -exec truncate -s 0 {}` or delete)
- **Apache Logs:** `/var/log/apache2/` (sudo required)
...
- **Composer config:** `wp-customer-ai-chatbot/composer.json`
- **Dev dependencies only** (PHPUnit, PHPCS, WP Stubs)
- **No runtime dependencies**
- **Local setup:** Run `composer install` in `wp-customer-ai-chatbot/` for local dev tools/linting
- **Distribution:** Exclude `vendor/` from plugin zip, or use `composer install --no-dev --optimize-autoloader` if runtime deps are added
...
- **Run tests:** SSH to server, run `phpunit` in the appropriate directory
...
- **Develop locally**
- **Deploy to staging** via SCP, rsync, or deploy script
- **After deploy:**
    - Clear PHP OPcache/server caches
    - Restart PHP-FPM/Apache
    - Verify file contents on server if changes don't appear
- **For full release and packaging steps, see:** [`../../../dev-tools/deployment/release-process.md`](../../../dev-tools/deployment/release-process.md)
...
**Note:** For extensive edits, always download, edit locally, and upload back to the server.
