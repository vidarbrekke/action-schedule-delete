...
jQuery(document).ready(function($) {
    ...
    function initChatbot() {
        ...
        function verifyAjaxConnectivity() {
            ...
            // Make a test request to the debug nonce endpoint
            $.ajax({
                ...
                success: function(response) {
                    console.log('WCAC: AJAX connectivity test successful', response);
                    
                    if (response.success) {
                        console.log('WCAC: Nonce verification: ' + response.data.verification_result);
                        
                        // If the nonce is invalid, refresh it
                        if (response.data.verification_result === 'invalid') {
                            console.warn('WCAC: Nonce is invalid, updating with new nonce');
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    ...
                    if (xhr.status === 400) {
                        ...
                        console.error('WCAC: 403 Forbidden error. Response:', xhr.responseText);
                    ...
                    console.error('WCAC: AJAX error', status, error, xhr.responseText);
                },
                complete: function() {
