...
## Overview
This document outlines the Test-Driven Development (TDD) approach for the WordPress Customer AI Chatbot plugin. The project now includes both automated (unit/integration) and manual tests for all major features. We will use the standard WordPress testing framework (WP_UnitTestCase based on PHPUnit) for unit and integration tests.
...
- **Unit Tests (`WP_UnitTestCase`):**
    ...
    - Test that all WooCommerce product types, including grouped and custom types, are supported and indexed. See [optimizer_parameters.md](../optimizer_parameters.md) for extension details.
    - Test that the Rating Weight parameter boosts products with higher average ratings. See [optimizer_parameters.md](../optimizer_parameters.md) for details.
- **Integration Tests:**
...
- **Unit Tests (`WP_UnitTestCase`):**
    ...
    - Test context compression modes (None, Title, Category, Fuzzy) for correct deduplication/clustering of context.
    - Test out-of-stock penalty application in scoring logic.
- **Integration Tests:**
...
- **Manual/E2E Testing:**
    ...
    - Test the effect of different context compression and out-of-stock penalty settings on retrieval and ranking.
...
All major test areas are now implemented and maintained as the codebase evolves.

- Ensure new parameters (context compression, out-of-stock penalty) are covered by both unit and integration tests as features evolve.
- All WooCommerce product types, including grouped and custom types, are supported and indexed. See [optimizer_parameters.md](../optimizer_parameters.md) for extension details.
- Always verify that new parameters (e.g., Rating Weight) are reflected in both the admin UI and optimizer documentation. 