...
// Get cart item key if it exists
$cart_item_key = isset($cart_item_key) ? $cart_item_key : '';
...
if (class_exists('WCQM_Rules_Manager')) {
    ...
    if (method_exists($rules_manager, 'get_product_multiple')) {
        // Force rules manager to load fresh rules for accurate values
        if (method_exists($rules_manager, 'load_rules')) {
            $rules_manager->load_rules(true);
        }
        
        // Get the product ID to check - use variation ID if available
        $check_product_id = $product_id;
        if (isset($args['cart_item']) && !empty($args['cart_item']['variation_id'])) {
            $check_product_id = $args['cart_item']['variation_id'];
        }
        
        // Get the multiple directly from the rules manager for consistency
        $product_multiple = $rules_manager->get_product_multiple($check_product_id);
        if ($product_multiple > 0) {
            $multiple = $product_multiple;
            
            // Get full product info for better debug logging
            $product = wc_get_product($check_product_id);
            $product_name = $product ? $product->get_name() : "Product #{$check_product_id}";
            $categories = $product ? $product->get_category_ids() : array();
            $category_string = implode(',', $categories);
            
            error_log("WCQM Debug: Cart item detected for product ID: {$check_product_id} ({$product_name}) - Using step value: {$multiple}");
            error_log("WCQM Debug: Product categories: {$category_string}");
            
            // If the input_id is a DOM element, log it for debugging
            if (is_string($input_id)) {
                error_log("WCQM Debug: Full input element: <input id=\"{$input_id}\">");
            }
        }
    }
}
