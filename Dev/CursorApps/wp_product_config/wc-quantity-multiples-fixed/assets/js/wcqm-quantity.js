...
(function($) {
     ...
     * @return {number} Multiple value
     */
    function getMultipleForProduct(productId) {
        // If product ID is invalid, return default
        if (!productId || isNaN(parseInt(productId))) {
            log('Invalid product ID, using default multiple: ' + defaultMultiple);
            return defaultMultiple;
        }
        
        productId = parseInt(productId);
        
        // First priority: Check if we have a pre-resolved multiple from the server
        // This is populated from server-side logic that correctly applies all rule priorities
        if (globalRules && globalRules.resolved_products && globalRules.resolved_products[productId]) {
            var resolvedMultiple = parseInt(globalRules.resolved_products[productId]);
            log('Found pre-resolved multiple for ' + productId + ': ' + resolvedMultiple);
            return resolvedMultiple;
        }
        
        // Check if this product has quantity_multiple data attribute directly on the element
        var $input = $('input.qty[data-product-id="' + productId + '"]');
        if ($input.length > 0 && $input.attr('data-multiple')) {
            var dataMultiple = parseInt($input.attr('data-multiple'));
            if (!isNaN(dataMultiple) && dataMultiple > 0) {
                log('Using multiple from data-multiple attribute: ' + dataMultiple);
                return dataMultiple;
            }
        }
        
        // Second: Check local cache for quick lookup
        if (productRules[productId]) {
            log('Using cached multiple for product ' + productId + ': ' + productRules[productId]);
            return productRules[productId];
        }
        
        // Third: Check for product-specific rule in global rules
        if (globalRules && globalRules.products && globalRules.products[productId]) {
            var multiple = parseInt(globalRules.products[productId]);
            productRules[productId] = multiple; // Cache for future use
            log('Found product-specific rule for ' + productId + ': ' + multiple);
            return multiple;
        }
        
        // Fourth: Check for category rules
        var categoryMultiple = getCategoryMultipleForProduct(productId);
        if (categoryMultiple) {
            log('Found category rule for product ' + productId + ': ' + categoryMultiple);
            productRules[productId] = categoryMultiple; // Cache for future use
            return categoryMultiple;
        }
        
        // Finally: Use default
        log('No rule found for product ' + productId + ', using default: ' + defaultMultiple);
        productRules[productId] = defaultMultiple; // Cache default for this product
        return defaultMultiple;
    }
    
    /**
     * Get category multiple for a product
     * This function checks if a product has categories with rules
     */
    function getCategoryMultipleForProduct(productId) {
        if (!globalRules || !globalRules.categories) {
            return null;
        }
        
        // Check if we have categories for this product
        if (globalRules.product_categories && globalRules.product_categories[productId]) {
            var categories = globalRules.product_categories[productId];
            
            // Check each category for a rule
            for (var i = 0; i < categories.length; i++) {
                var categoryId = categories[i];
                if (globalRules.categories[categoryId]) {
                    var multiple = parseInt(globalRules.categories[categoryId]);
                    log('Found multiple for product ' + productId + ' from category ' + categoryId + ': ' + multiple);
                    return multiple;
                }
            }
        }
