...
# Other imports and directory setup remain unchanged
...
def sma_strategy(data, short_period, long_period):
    ...
    print(f"DEBUG: Signal column '{signal_column}' generated. Sample:\n{data_with_indicators[[signal_column, position_column]].head()}")
...
# Consensus Strategy Function with Debugging
def consensus_strategy(data, strategies):
    print("DEBUG: Executing consensus strategy")
    ...
    consensus_trades = data[data['Consensus_Position'] != 0]
    print(f"DEBUG: Number of consensus trades made: {len(consensus_trades)}")
    if len(consensus_trades) > 0:
        print(f"DEBUG: Sample consensus trades:\n{consensus_trades[['Consensus_Signal', 'Consensus_Position']].head()}")
    return data

# calculate_raw_score Function remains unchanged
...
def backtest_sma(symbol, start_date, end_date, short_period, long_period):
    ...
    print(f"DEBUG: Backtest Result for {symbol} ({short_period}, {long_period}): {performance}")
