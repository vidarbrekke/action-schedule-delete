...
def calculate_raw_score(data, signal_column):
    score = 0
    for signal in data[signal_column]:
        if signal == 1:  # Assuming '1' represents a buy signal
            score += 1
        elif signal == -1:  # Assuming '-1' represents a sell signal
            score -= 1
    return score
...
if __name__ == '__main__':
    ...
    for short_period in short_periods:
        for long_period in long_periods:
            if short_period < long_period:
                # SMA strategy calculation
                data_with_sma, signal_column, position_column = sma_strategy(data, short_period, long_period)

                # Performance and raw score calculation
                performance = calculate_performance(data_with_sma, position_column)
                raw_score = calculate_raw_score(data_with_sma, position_column)  # Corrected line
    ...
    consensus_plot_path = plot_strategy(data_with_consensus, symbol, 'Consensus_Strategy', consensus_performance, None, 10, 50, plots_dir)
