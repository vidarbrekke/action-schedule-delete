...
def plot_strategy_performance(data, strategy_name, strategy_params, metrics, ticker, timestamp):
    ...
    # Ensure signals are plotted on the top layer with no transparency
    ...
    for key, value in metrics.items():
        ...
        color = 'green' if key == 'final_balance' and value > initial_balance else 'black'
...
def backtest_strategy(data, ticker, strategy_func, from_config=True, strategy_params=None):
    ...
    if from_config:
        ...
        strategies = {strategy_params['strategy_name']: strategy_params}
    ...
    for strategy_name, params in strategies.items():
        ...
        signal_strength = params.get('signal_strength', 1)

        # Dynamic parameter adjustment
        data_with_regime = identify_market_regime_enhanced(data)
        latest_regime = data_with_regime['regime'].iloc[-1]
        adjusted_params = adjust_risk_parameters(params, latest_regime)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', type=str, required=False, help='Path to the test configuration JSON file')
    parser.add_argument('--ticker', type=str, required=False, help='Ticker symbol for the backtest')
